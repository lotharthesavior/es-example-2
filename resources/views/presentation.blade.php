...
<head>
  ...
  <style>
    ...
    .reveal .accent-2 { color: var(--accent-2); }
    .reveal .muted { color: #888; font-size: 0.7em; }
    .reveal .big-number { font-size: 4em; font-weight: 800; color: var(--accent-2); line-height: 1; }
    .reveal .flex { display: flex; gap: 2rem; align-items: center; justify-content: center; }
    .reveal .card {
      background: #1a1a1a;
    ...
    .reveal pre { box-shadow: none; font-size: 0.5em; }
    .reveal pre code { padding: 1rem; border-radius: 8px; }
    .reveal ul { list-style: none; padding: 0; }
    .reveal ul li { margin: 0.5em 0; padding-left: 1.2em; position: relative; }
    .reveal ul li::before { content: "▸"; color: var(--accent); position: absolute; left: 0; }
    .reveal .arrow { font-size: 2em; color: var(--accent); }
    .reveal .node {
      background: #1e2a3a;
    ...
    .reveal .col-label.alt { color: var(--accent-2); }
    .reveal footer {
      position: fixed;
      bottom: 12px;
      left: 20px;
...
<body>
  <div class="reveal">
    <div class="slides">
      ...
      <!-- Slide 3: A Friday Afternoon -->
      <section>
        <h2>A Friday Afternoon…</h2>
        <blockquote style="margin-top: 1.5em; font-size: 0.85em;">
          Your team ships an update. By Monday, <span class="accent-2">10,000 order statuses</span>
          silently flipped from <em>processing</em> to <em>shipped</em> — but nothing actually shipped.
        </blockquote>
        <p class="muted" style="margin-top: 2em;">What was the correct state? You don't know. The history is gone.</p>
      </section>
      ...
      <section>
        ...
        <ul style="font-size: 0.9em;">
          ...
          <li>You know <strong>what is</strong>. Not <strong>what was</strong>. Never <strong>why</strong>.</li>
          <li>History tables capture snapshots — not <span class="accent">intent</span></li>
        </ul>
      </section>

      <!-- Slide 5: Domains that feel this pain -->
      <section>
        <h2>Where This Hurts</h2>
        <div style="margin-top: 1.5em;">
          <span class="card"><strong class="accent">Banking</strong><br><span class="muted">balance is derived</span></span>
          <span class="card"><strong class="accent">Healthcare</strong><br><span class="muted">vitals over time</span></span>
          <span class="card"><strong class="accent">Inventory</strong><br><span class="muted">why did stock change?</span></span>
          <span class="card"><strong class="accent">E-commerce</strong><br><span class="muted">where is order #4521?</span></span>
        </div>
        <p class="muted" style="margin-top: 1.5em;">Every domain answers <em>"what is?"</em> — but struggles with <em>"how did we get here?"</em></p>
      </section>

      <!-- Slide 6: Talk Objective -->
      <section>
        <h2>Talk Objective</h2>
        <ol style="font-size: 0.9em;">
          <li><strong class="accent">What</strong> is event sourcing — and why now?</li>
          <li><strong class="accent">How</strong> to apply it in PHP <span class="muted">(Laravel + Spatie)</span></li>
          <li><strong class="accent">Where</strong> it pays off — and where it doesn't</li>
        </ol>
      </section>

      <!-- Slide 7: Traditional vs Event-Sourced -->
      <section>
      ...
      <!-- Slide 8: PM walks over -->
      <section>
        <h2>"What happened to <span class="accent-2">order #4521</span>?"</h2>
        <div class="two-col" style="margin-top: 1em;">
          <div>
            <div class="col-label alt">Traditional</div>
            <p style="font-size: 0.85em;">Status: <code>shipped</code>.</p>
            <p class="muted">…that's all you have.</p>
          </div>
          <div>
            <div class="col-label">Event-Sourced</div>
            <p style="font-size: 0.75em;">Placed 14:00 → Paid 14:05<br>Picked 15:15 → Shipped 16:30</p>
            <p class="muted">Every step. Every timestamp.</p>
          </div>
        </div>
      </section>

      <!-- Slide 9: Mindset shift -->
      <section>
      ...
      <!-- Slide 10: Core Concepts overview -->
      <section>
        <h2>Core Concepts</h2>
        <div style="margin-top: 1.5em;">
      ...
      <!-- Slide 18: Live Code intro -->
      <section>
        <h2>Live Code</h2>
        <p style="font-size: 1.2em;">Health Metrics in <span class="accent-2">Laravel + Spatie</span></p>
        <p class="muted" style="margin-top: 1.5em;">Heart rate, blood pressure, oxygen — events all the way down.</p>
      </section>

      <!-- Slide 19: Code - The event -->
      <section>
        <h3>The Event</h3>
...
PatientAggregate::retrieve($patientId)
    ->replay(until: '2026-01-15 09:00');</code></pre>
      ...
      <section>
        ...
        <p class="muted" style="margin-top: 1.5em;">A duplicate <code>PaymentReceived</code> is not a free coffee.</p>
      </section>
...
PatientAggregate::fake($patientId)
    ...
    ->assertRecorded([
      ...
      <!-- Slide 29: CQRS -->
      <section>
        <h2>The <span class="accent-2">CQRS</span> Connection</h2>
        <div class="two-col" style="margin-top: 1em;">
          <div>
            <div class="col-label">Commands</div>
            <p style="font-size: 0.8em;">Aggregates handle writes. Validate rules. Emit events.</p>
          </div>
          <div>
            <div class="col-label alt">Queries</div>
            <p style="font-size: 0.8em;">Projections handle reads. Optimized per use case.</p>
          </div>
        </div>
        <p class="muted" style="margin-top: 1.5em; font-size: 0.8em;">If you do event sourcing with projections — you're already doing CQRS.</p>
      </section>

      <!-- Slide 30: Migration / Strangler Fig -->
      <section>
        ...
        <ul style="font-size: 0.85em;">
          ...
          <li>Event-sourced aggregates live next to Eloquent models</li>
          <li>Same database is fine — just write events, not rows</li>
        </ul>
      </section>
      ...
      <section>
        <h2>Resources</h2>
        <ul style="font-size: 0.75em;">
          <li><a href="https://github.com/lotharthesavior/es-example">github.com/lotharthesavior/es-example</a></li>
          <li><a href="https://spatie.be/docs/laravel-event-sourcing">spatie.be/docs/laravel-event-sourcing</a></li>
          <li>Greg Young — <em>CQRS &amp; Event Sourcing</em> (talks)</li>
          <li>Martin Fowler — <em>Event Sourcing</em> pattern</li>
        </ul>
        <p class="muted" style="margin-top: 1.5em; font-size: 0.7em;">
          Predecessor talk: Longhorn PHP 2025 — <em>Building Resilient PHP Applications with an Event-Driven Mindset</em>
        </p>
      </section>
